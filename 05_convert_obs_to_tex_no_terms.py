...
OBS_FILE =  'thesis expose 4th draft.md'
OUT_DIR = 'output'
OUT_TEX =  'expose.md'
...
def main():
    ...
    with open(os.path.join(OBS_PATH, OBS_FILE), 'r') as f:
        ...
        # definitions
        obs, _, missing_definitions = expand_terms(obs)
...
def expand_terms(text):
    # actually just kills the md shit
    ...
    for link in all_links:
        ...
        text = text.replace(f'[[{link}]]', used_form)
...
def expand_acronyms(text):
    ...
    for link in all_links:
        split = link.split('|')
        id = split[-1]
        # replace all whitespaces and non alphanumeric with, also make all letters uppercase
        id_cleaned = re.sub(r'\W+', '', id).upper()
        used_form = split[0]
        # for each of the words in used_form, capitalize first letter
        # words can be separated by spaces or dash!
        # ...and we want to *keep* the correct separator
        used_form = re.sub(r'(\b[a-z])', lambda x: x.group(1).upper(), used_form)
        ...
        # print("added entry", glossary_entry)
